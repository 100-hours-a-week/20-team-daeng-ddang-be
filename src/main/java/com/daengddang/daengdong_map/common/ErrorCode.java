...
public enum ErrorCode {
    ...
    DOG_FACE_NOT_RECOGNIZED(HttpStatus.UNPROCESSABLE_CONTENT,
            "영상에서 강아지 얼굴을 인식할 수 없습니다."),
    CHAT_ANALYSIS_NOT_SUPPORTED(HttpStatus.UNPROCESSABLE_CONTENT,
            "해당 채팅 이미지 및 텍스트를 분석할 수 없습니다."),
    AI_SERVER_BAD_REQUEST(HttpStatus.BAD_GATEWAY,
            "AI 서버가 요청을 처리할 수 없는 형식입니다."),
    AI_SERVER_INTERNAL_ERROR(HttpStatus.BAD_GATEWAY,
            "AI 서버 처리 중 오류가 발생했습니다."),
    AI_SERVER_RESPONSE_INVALID(HttpStatus.BAD_GATEWAY,
            "AI 서버 응답 형식이 올바르지 않습니다."),
    AI_SERVER_TIMEOUT(HttpStatus.GATEWAY_TIMEOUT,
