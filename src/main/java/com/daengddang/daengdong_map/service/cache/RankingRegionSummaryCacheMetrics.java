...
@Component
@Getter
public class RankingRegionSummaryCacheMetrics {
    ...
    public RankingRegionSummaryCacheMetrics(ObjectProvider<MeterRegistry> registryProvider) {
        ...
        hitCounter = Counter.builder("cache.ranking.region.summary.hit")
                .description("Region ranking summary cache hit count")
                .register(registry);
        missCounter = Counter.builder("cache.ranking.region.summary.miss")
                .description("Region ranking summary cache miss count")
                .register(registry);
        bypassDisabledCounter = Counter.builder("cache.ranking.region.summary.bypass.disabled")
                .description("Region ranking summary cache bypass count when cache is disabled")
                .register(registry);
        fallbackErrorCounter = Counter.builder("cache.ranking.region.summary.fallback.error")
                .description("Region ranking summary cache fallback-to-db count due to cache error")
                .register(registry);
        dbLoadCounter = Counter.builder("cache.ranking.region.summary.db.load")
                .description("Region ranking summary DB load count when cache miss/fallback occurs")
                .register(registry);
        readErrorCounter = Counter.builder("cache.ranking.region.summary.read.error")
                .description("Region ranking summary cache read error count")
                .register(registry);
        writeErrorCounter = Counter.builder("cache.ranking.region.summary.write.error")
                .description("Region ranking summary cache write error count")
                .register(registry);
    }
    ...
    private void maybeLog() {
        ...
        if (total > 0 && total % LOG_EVERY == 0) {
            ...
            log.info(
                    "[RankingRegionSummaryCacheMetrics] hit={}, miss={}, hitRatio={}%, bypassDisabled={}, fallbackError={}, dbLoad={}, readError={}, writeError={}",
                    hitCount,
                    missCount,
